...
export class EditRoomModalComponent implements OnInit {
  ...
  khoiTaoForm() {
    this.addRoomForm = new UntypedFormGroup({
      roomName: new UntypedFormControl(this.room.name, [Validators.required, Validators.maxLength(100)])
    })
  }

  save(){
    this.roomService.editRoom(this.room.id, this.addRoomForm.value.roomName).subscribe((res: Room)=>{
      if(res){
